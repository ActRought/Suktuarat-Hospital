...
const Navbar = () => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/api/patients/${patientId}/appointments`);
  ...
  const handleCancelAppointment = async (appointId) => {
    if (window.confirm("คุณแน่ใจหรือไม่ว่าต้องการยกเลิกคิวนี้?")) {
      try {
        await axios.put(`http://localhost:5000/api/appointments/${appointId}/cancel`);
