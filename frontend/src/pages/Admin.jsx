...
const Admin = () => {
    ...
    // เพิ่มตัวแปรมารับค่า Dashboard Stats ตรงๆ จาก API
    const [stats, setStats] = useState({
        totalPatients: 0,
        completed: 0,
        waiting: 0,
        doctorsActive: 0 // จะรับค่า totalDoctors จาก Backend มาใส่ตรงนี้
    });
    ...
    if (role !== 'Admin' && role !== 'admin') {
        return (
            <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
                ...
                <h2 className="text-3xl font-bold text-gray-800 mb-2">ไม่มีสิทธิ์เข้าถึง</h2>
            </div>
        );
    }
        ...
        <div className="max-w-7xl mx-auto py-6 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">ข้อมูลสถิติอัปเดตล่าสุดจากระบบ</p>
                </div>
                <div className="relative">
                    ...
                    <input
                        ...
                        className="pl-10 pr-4 py-2.5 rounded-full border border-gray-200 bg-white focus:ring-2 focus:ring-blue-500 outline-none w-full md:w-72 shadow-sm"
                    />
                </div>
            </div>
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-gray-500 text-sm font-medium">ผู้ป่วยเข้ารับบริการวันนี้</p>
                                <h3 className="text-2xl font-bold text-gray-800">{stats.totalPatients} <span className="text-sm font-normal text-gray-400">คน</span></h3>
                            </div>
                        </div>

                        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex items-center gap-4">
                            <div className="bg-emerald-100 p-4 rounded-2xl text-emerald-600"><CheckCircle2 size={28} /></div>
                            <div>
                                <p className="text-gray-500 text-sm font-medium">ตรวจเสร็จสิ้นแล้ว</p>
                                <h3 className="text-2xl font-bold text-gray-800">{stats.completed} <span className="text-sm font-normal text-gray-400">คน</span></h3>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-gray-500 text-sm font-medium">คิวรอตรวจ (Waiting)</p>
                                <h3 className="text-2xl font-bold text-gray-800">{stats.waiting} <span className="text-sm font-normal text-gray-400">คน</span></h3>
                            </div>
                        </div>

                        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex items-center gap-4">
                            <div className="bg-purple-100 p-4 rounded-2xl text-purple-600"><Stethoscope size={28} /></div>
                            <div>
                                <p className="text-gray-500 text-sm font-medium">แพทย์ที่ออกตรวจ</p>
                                <h3 className="text-2xl font-bold text-gray-800">{stats.doctorsActive} <span className="text-sm font-normal text-gray-400">ท่าน</span></h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                            <h3 className="font-bold text-gray-800 flex items-center gap-2 mb-6">
                                <Activity className="text-emerald-500" size={20} /> ความหนาแน่นแต่ละแผนก
                            </h3>
                            <div className="space-y-5">
                                {deptDensity.length === 0 ? (
                                    <p className="text-gray-500 text-sm text-center py-4">ยังไม่มีคิวตรวจในวันนี้</p>
                                ) : (
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="p-6 border-b border-gray-100 bg-slate-50/50 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                <CalendarCheck className="text-blue-500" size={20} /> คิวตรวจทั้งหมด
                            </h3>
                            <button onClick={fetchDashboardData} className="text-sm text-blue-600 font-medium hover:underline bg-blue-50 px-3 py-1 rounded-md">
                                รีเฟรชข้อมูล
                            </button>
                        </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-50 text-gray-500 text-sm border-b border-gray-100">
                                        ...
                                        <th className="py-4 px-6 font-medium">ชื่อผู้ป่วย</th>
                                        <th className="py-4 px-6 font-medium">แผนก / แพทย์</th>
                                        <th className="py-4 px-6 font-medium">สถานะ</th>
                                        <th className="py-4 px-6 font-medium text-center w-32">จัดการ</th>
                                    </tr>
                                </thead>
                                    ...
                                    {filteredAppointments.length === 0 ? (
                                        ...
                                        filteredAppointments.map((app) => (
                                            <tr key={app.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="py-4 px-6 text-gray-800">{app.patient}</td>
                                                <td className="py-4 px-6">
                                                    <p className="text-gray-800 font-medium">{app.dept}</p>
                                                    <p className="text-xs text-gray-500">{app.doctor}</p>
                                                </td>
                                                <td className="py-4 px-6">
                                                    {app.status === 'ยืนยันแล้ว' && <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-600 text-xs font-bold border border-blue-100">ยืนยันแล้ว</span>}
                                                    {app.status === 'ตรวจเสร็จสิ้น' && <span className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 text-xs font-bold border border-emerald-100">ตรวจเสร็จสิ้น</span>}
                                                    {app.status === 'ยกเลิก' && <span className="px-3 py-1 rounded-full bg-red-50 text-red-600 text-xs font-bold border border-red-100">ยกเลิกแล้ว</span>}
                                                </td>
                                                <td className="py-4 px-6 text-center">
                                                    {app.status === 'ยืนยันแล้ว' ? (
                                                        editingId === app.id ? (
                                                            <select
                                                                className="border border-gray-300 rounded-md px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
                                                                onChange={(e) => {
                                                                    if (e.target.value) {
                                                            ...
                                                            <button
                                                                ...
                                                                className="text-sm font-medium text-blue-600 border border-blue-200 hover:bg-blue-50 px-4 py-1.5 rounded-lg transition-colors w-full"
                                                            >
                                                                แก้ไข
                                                            </button>
                                                        )
